...
# Phase 5 — Polish（2026-09-03 完成）
按 spec §22 Phase 5：灯光 / 材质 / UI / 镜头动画 / 响应式 / 性能 / 可访问性。

## 完成内容
- **PBR 材质升级**：
  - 球台：ITTF 比赛级微磨砂漆面 Physical 材质 + 底底加固裙边框架 (skirt) + 支撑钢架连杆 + 15mm 顶部包边球网与金属固定夹扣
  - 乒乓球：40+ ABS 比赛级微磨砂透明高光感 Physical 材质 + 保留并增强 contact squash 挤压形变
  - 球拍：红黑双色反胶表面 + 橙蓝高弹海绵 + 五层纯木夹层底板侧影 + 黑色边缘护边带 + 人体工学 FL 拼木收腰手柄与金属底标
- **接触软阴影 (Contact Shadows)**：
  - 引入 `@react-three/drei` 的 ContactShadows 投射于地面，消除悬浮突兀感
- **Web Audio 纯程序化击球音效**：
  - `src/audio/sound.ts`：纯浏览器原生 Web Audio API 合成，零外部音频资源依赖
  - 落台 Pong（低频木腔共鸣）、击拍 Ping（高频海绵与底板爆鸣，随转速微调）、擦网 Tick
  - 支持一键开关与快捷键静音 (M)，提供完整单测防崩与优雅降级
- **全键盘快捷键与操作指南**：
  - `Space` 播放/暂停、`R` 重置发球、`.` / `→` 单步微调 (1/600s)、`1`~`5` 快捷机位切换、`M` 音效、`?` / `H` 打开快捷键指南、`Esc` 取消选择/退出场景
  - `ShortcutsModal.tsx`：优雅清晰的快捷键指南弹窗与 ARIA 支持
- **响应式与面板折叠**：
  - 左右面板支持一键收起/展开折叠，最大化 3D 沉浸式视口
  - 响应式媒体查询：移动端与平板自适应布局与滚动支持
- **测试与质量**：
  - 9 组测试文件，74/74 项单元测试 100% 通过，`tsc --noEmit` 0 错误，`pnpm build` 成功通过
